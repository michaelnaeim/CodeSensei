    ...
    <div className="grid lg:grid-cols-5 gap-4">
        ...
        <div className="overflow-auto max-h-[440px] font-mono text-[12px] leading-6 bg-[var(--bg-code)]">
          {lines.map((line, i) => {
            ...
            return (
              ...
              >
                <span className="text-zinc-500 w-6 text-right select-none shrink-0">{lineNum}</span>
      ...
      <div className="lg:col-span-2 panel p-4 flex flex-col">
        ...
        {activeLine && noteMap[activeLine] ? (
          <>
            <p className="text-xs font-mono text-[var(--text-muted)] mb-2">Line {activeLine}</p>
            <p className="text-sm leading-relaxed">{noteMap[activeLine]}</p>
          </>
        ) : (
          ...
          {notes.map((n) => (
            <button
              ...
              className={cn(
                ...
                activeLine === n.line ? "bg-[var(--accent-soft)]" : "hover:bg-[var(--bg-muted)]"
              )}
            >
              <span className="font-mono text-[var(--accent)]">L{n.line}</span>
              <span className="text-[var(--text-secondary)] ml-2">{n.text}</span>
            </button>
          ))}
        </div>
        <Button variant="primary" className="w-full mt-4" onClick={onComplete}>
