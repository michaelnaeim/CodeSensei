...
/**
 * Hub-and-spoke layout: the first (root) concept sits at top center and
 * radiates curved edges to every other concept laid out in an arc/grid below.
 * Sequential concepts are linked as prerequisites.
 */
function buildKnowledgeGraph(
  ...
  const nodes: Node[] = topics.map((t, i) => {
    ...
    if (i === 0) {
      ...
      const idx = i - 1;
      x = (idx % perRow) * 220;
      y = (Math.floor(idx / perRow) + 1) * (opts.hierarchy ? 200 : 180) + 60;
    }
    return {
      id: t.id,
      type: "conceptNode",
  ...
  for (let i = 1; i < topics.length - 1; i++) {
    edges.push({
      id: `seq-${topics[i].id}-${topics[i + 1].id}`,
      source: topics[i].id,
